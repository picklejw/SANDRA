...
pub async fn subscribe(source: Source, on_event: fn(ev: &NotificationMessageHolderType)) -> fn() -> Result<bool, ParseIntError> {
    let camera_ip = "10.0.20.232:8000";
    let username = source.username;
    let password =  source.password;


    async fn do_authenitcate(username: &'static str, password: &'static str, camera_ip: &'static str) -> (Client, PullMessages) {
    ...
    let (mut pull_msg_client, mut pull_messages_request) = do_authenitcate(&username, &password, &camera_ip).await;
      ...
      if Some(termination_time).is_some() {
        let lookAheadTime =  Utc::now() + Duration::seconds(1);
          if (termination_time < lookAheadTime) {
              let (new_pull_msg_client, new_pull_messages_request) = do_authenitcate(&username, &password, &camera_ip).await;
